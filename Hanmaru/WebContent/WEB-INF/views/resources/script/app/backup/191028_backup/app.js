	...
	function callDomainList(){
		...
		$http(param).success(function(data) {
			...
			if(code == 1){
				...
				$s.generalLogin_domain = $s.domainList[0];// halla.com
															// //hd-bsnc.com
...
// alert($s.settingPinNumber);
	...
	$s.performGeneralLogin = function() {
		...
		if(autoLoginCheck==true){
			loginData = {
				userID:$s.appUserId==''?$s.general_id:$s.appUserId,
				autoLogin:'Y'
			};
		}else{
			loginData = {
...
// chatbotLogin();
	...
	function autoLogin(){
		...
		var loginData = { userID:$s.appUserId, DeviceID:$rs.deviceID,
		 AppVersion:$rs.appVersion, AppType:'' };
		 

		/*var loginData = {
				userID:'hs1.kim',
				DeviceID:'f2bfabb2329bc82b',
				AppVersion:'100',
				AppType:'hanmaru'
			};*/
//		alert("userID:"+$s.appUserId+"\n"+
//				"DeviceID:"+$rs.deviceID+"\n"+
//				"AppVersion:"+$rs.appVersion+"\n"
//		);
		...
		$http(param).success(function(data) {
			...
			if(code == 1){
				...
				if(autoLoginInfo.AutoLogin=='Y'){
					$s.general_id=$s.appUserId;
					$s.general_pw=autoLoginInfo.Password;
					$s.isPinLogin=false;
					accessUser();
				}
...
// alert($s.inputPinNumber);
		...
		$http(param).success(function(data) {
			...
			if(code == 1){
				...
				$rs.chatbotUserId = $s.isPinLogin ? $s.appUserId+'@'+ $s.generalLogin_domain : $s.general_id+'@'+$s.generalLogin_domain; // 소문자
																																			// 처리할것.
