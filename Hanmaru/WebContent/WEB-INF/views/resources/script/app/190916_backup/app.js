...
appHanmaru.controller('loginController', ['$scope', '$http', '$rootScope', function($s, $http, $rs) {
	...
	$s.pin_input_id = '';
	$s.pin_input_pwd = '';
	$rs.isPinLogin = true; //간편 핀번호 로그인 여부
...
//	$rs.apiURL = "https://ep.hd-bsnc.com";
	...
	$s.general_id = '';//kyungho.lee //jina
	$s.general_pw = '';//777sapden3 //qw123321
...
//			//핀번호 로그인을 위한 아이디 불러오기
	...
	//핀번호 로그인을 위한 아이디 불러오기
	window.getAndroidPinLoginUserId = function(userId){
		$s.appUserId = userId;
	};
	
	getIosPinLoginUserId = function(userId){
		$s.appUserId = userId;
	}
...
//			alert($s.inputPinNumber);
		...
		$http(param).success(function(data) {
			...
			if(code == 1){
				...
				$rs.chatbotUserId = $s.isPinLogin ? $s.appUserId+'@'+ $s.generalLogin_domain : $s.general_id+'@'+$s.generalLogin_domain; //소문자 처리할것.
	...
	function parseCIDAttachMailData(mail) {
		...
		if(img.length > 0) {
			...
			if(approvalLinkIndex != -1) {
				...
				setTimeout(function(){
					var script = "javascript:btnToApprovalDetail('"+docID.trim()+"');";
					mail.Body = mail.Body.replace("https://ep.halla.com/Workflow/Page/Link.aspx?docId="+docID, script);//$rs.apiURL이 현재 https.
