// mainController
appHanmaru.controller('hallaMainController', ['$scope', '$http', '$rootScope', '$timeout', function($s, $http, $rs, $timeout){
	$s.headSubject = '';
	$s.headImage = '';
	$s.bannerImage = '';
	...
	$rs.$on('initMandoMain', function(event) {
		...
		$http(param).success(function(data) {
			...
			//여기서 계열사별 구분 예정.
			$rs.$broadcast('initHallaMainList',boardData);
...
//			pushPage(pageName, 'pg_main_list');
			...
			pushPage(pageName, 'pg_main_halla_view');
		});
	});
	
	$rs.$on('initHallaMainList',function(event,boardData){
