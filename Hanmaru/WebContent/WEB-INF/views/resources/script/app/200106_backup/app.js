...
var appHanmaru = angular.module('appHanmaru', ['ngSanitize','ngRoute','ngAnimate','oc.lazyLoad','slip'])/*'ngTouch',*//*,'hmTouchEvents',*/
...
//	  '/resources/script/lib/pinchZoom/jquery.hammer.min.js',
...
//		pushPage(pageName, 'pg_' + menuName + '_list');
		...
		if(menuName === 'gqms'){
			if($rs.agent == 'android'){
				if(androidWebView != undefined) { 
					androidWebView.deepLinkGqmsApp();
				}
			}else if($rs.agent=='ios') {
			...
			if($rs.agent == 'android'){
				...
				webkit.messageHandlers.deepLinkIosHallaInApp.postMessage($rs.pmSabun);
			}
		}
...
//			var tmp = mainData.Head;

			$s.hallaInList = mainData.HallaMessage; //19.11.25 변경
//			$s.hallaInList.splice(0, 0, tmp); //메인상단 기사 중간 배너영역에 임시 push
...
// 팝업 세팅
	...
	$s.selectStartTime = function(){
		...
		setTimeout(function(){
			$s.$apply(function(){
				var scrollHeight = $('#attendanceTimeList').prop('scrollHeight');
				var scrollUnit = scrollHeight/24;
				var height =  scrollUnit * moment().format('HH') 
				
				$('#attendanceTimeList').scrollTop(height-150);
			},50);
		});
	};
	...
	$s.selectEndTime = function(){
		...
		setTimeout(function(){
			$s.$apply(function(){
				var scrollHeight = $('#quittingTimeList').prop('scrollHeight');
				var scrollUnit = scrollHeight/24;
				var height =  scrollUnit * moment().format('HH') 
				
				$('#quittingTimeList').scrollTop(height-150);
			},50);
		});
	};
...
// $s.searchSel = '제목';
	...
	$s.afterSwipe = function(e,idx){
		var email = $s.mailList[idx];
		$s.btnDlgMailDelete(e,email.ItemId);
	}
...
//		console.log('스와이프 삭제');
	...
	function deleteMail(swipedItem){
		...
		$s.btnConfirmDeleteMail = function(){
			...
			var arrMail = new Array();
			//스와이프 해서 삭제
			if(swipedItem !=null && swipedItem !=undefined && swipedItem.length > 0){
				arrMail.push(swipedItem);
			}
			//편집모드에서 삭제
			else{
				for(var idx in $s.mailEditList) {
				...
				setTimeout(function(){
					...
					setTimeout(function(){
						$s.$apply(function(){
							...
							$('.slideMenuMailList').scrollTop((scrollHeight/2) - 100); //조직도 스크롤 영역
						},50);
					});
				}, 200);
...
//		$s.txtCurrentDate = mnt.format('YYYY-MM-DD');
...
//		}
		...
		console.log('init work list param : ' , param);
