// mainController
appHanmaru.controller('holdingsMainController', ['$scope', '$http', '$rootScope', '$timeout', function($s, $http, $rs, $timeout){
	$s.headSubject = '';
	$s.headImage = '';
	$s.bannerImage = '';
	...
	$rs.$on('initHoldingsMain', function(event) {
		...
		$http(param).success(function(data) {
			...
			//여기서 계열사별 구분 예정.
			$rs.$broadcast('initHoldingsMainList',boardData);
...
//			pushPage(pageName, 'pg_main_list');
			...
			pushPage(pageName, 'pg_main_holdings_view');
		});
	});
	
	$rs.$on('initHoldingsMainList',function(event,boardData){
