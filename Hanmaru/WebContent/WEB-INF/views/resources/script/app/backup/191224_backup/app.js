...
var appHanmaru = angular.module('appHanmaru', ['ngSanitize','ngRoute','ngAnimate','oc.lazyLoad','hmTouchEvents'])/*'ngTouch','awesome-angular-swipe'*/
...
//				var localLang = sessionStorage.getItem("language");
	...
	$rs.slideMenuClick = function($event, isShow) {
		//2019.12.20 추가 - 배너 이미지 스크롤시 사이드 메뉴 노출 안되도록.
...
//		pushPage(pageName, 'pg_' + menuName + '_list');
		...
		if(menuName === 'gqms'){
			...
			if($rs.agent == 'android'){
				...
				webkit.messageHandlers.deepLinkIosMidasHelpApp.postMessage();
			}
		}else{
			if(menuName === 'reserv'){
...
//			$s.hallaInList = mainData.HallaIn; //19.11.25 변경
...
// $s.searchSel = '제목';
	...
	$s.onSwipeLeft = function() {
		
		$s.$apply(function() {
			console.log('111');
			$s.swipeAction = 'swipe left';
		});
		$timeout(_idle, 1000);
	};
	$s.onSwipeRight = function() {
		console.log('222');
		$s.$apply(function() {
			console.log(idx);
			$s.swipeAction = 'swipe right';
		});
		$timeout(_idle, 1000);
	};
...
//				}
	...
	$s.mailListBoxSelect = function(e, index, mail) {
		var currSelectedElement = angular.element('.mailListBox').eq(index).find('.swiper-content');
			// 메일 편집
			$rs.mailEditClick = !$s.mailEditClick;
