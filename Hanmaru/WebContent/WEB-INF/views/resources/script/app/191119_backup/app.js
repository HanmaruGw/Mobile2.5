...
appHanmaru.controller('mainController', ['$scope', '$http', '$rootScope', '$sce',function($s, $http, $rs, $sce) {
	...
	$rs.currentDetailPageId = '_include/splash';
...
appHanmaru.controller('ReservViewController', ['$scope', '$http', '$rootScope', '$timeout', function($s, $http, $rs, $timeout){
	...
	$s.btnResourceInfo = function(resourceInfo){
		...
		$rs.isDetailPage = true;
		$rs.currentDetailPageId = 'reservation/reservation_detailview'
...
//		$rs.currentPageId = 'reservation/reservation_detailview'; // 추가 페이지 변경
	...
	$s.reservCancel = function(seq){
		...
		$http(param).success(function(data){
			...
			$rs.isDetailPage = true;
			$rs.currentDetailPageId = 'reservation/reservation_detail'
...
//		$rs.currentDetailPageId = 'reservation/reservation_detail'
		...
		nextPageData($rs,'initReservBookingDetail','ReservBookingDetail',$s.areaCode,reservPossibleItem,$s.startDateTime,$s.endDateTime); // 페이지 변경될때 데이터이동되도록 함수설정
...
// $s.org_first_alt_user_list = result.Users;
	...
	$s.btnApplySelectedUser = function(e){
		...
		if( $s.attendType == 'cal_Attendee' ) {
			...
			nextPageData($rs,'Cal_Apply_Attendee','scheduleWrite', $s.arr_cal_attendee); // 페이지 변경될때 데이터이동되도록 함수설정
		}
		else if( $s.attendType == 'cal_Jointowner' ) {
			$rs.currentPageId = 'work_diary/schedule/schedule_write';
			// $rs.$broadcast("Cal_Apply_Jointowner", $s.arr_cal_Jointowner);
			nextPageData($rs,'Cal_Apply_Jointowner','scheduleWrite', $s.arr_cal_Jointowner); // 페이지 변경될때 데이터이동되도록 함수설정
		}
		else if( $s.attendType == 'ShareUser' ) {
			$rs.currentPageId = 'work_diary/schedule/diary_schedule'
			// $rs.$broadcast("Apply_ShareUser", $s.arr_shareUser);
			nextPageData($rs,'Apply_ShareUser','diaryScheduel', $s.arr_shareUser); // 페이지 변경될때 데이터이동되도록 함수설정
		} 
		else if($s.attendType == 'workReport'){
			$rs.currentPageId = 'work_diary/report/report_write'
			// $rs.$broadcast("workReportApplySelectedUser",
			// $s.arr_selected_rcv);
			nextPageData($rs,'workReportApplySelectedUser','reportWrite', $s.arr_selected_rcv); // 페이지 변경될때 데이터이동되도록 함수설정
		}
			...
			nextPageData($rs,'workPlanApplySelectedUser','planWrite',$s.arr_selected_rcv); // 페이지 변경될때 데이터이동되도록 함수설정
		}
			...
			// $s.arr_selected_rcv);
			nextPageData($rs,'workTaskApplyAttendanceUser','workWrite',$s.arr_selected_rcv); // 페이지 변경될때 데이터이동되도록 함수설정
		}
		else if($s.attendType == 'workTaskShare'){
			$rs.currentPageId = 'work_diary/work/work_write'
			// $rs.$broadcast("workTaskApplyShareUser", $s.arr_selected_rcv);
			nextPageData($rs,'workTaskApplyShareUser','workWrite',$s.arr_selected_rcv); // 페이지 변경될때 데이터이동되도록 함수설정
		}
		else {
//			$rs.currentPageId = 'reservation/reservation_detail'
			 $rs.$broadcast("reservApplySelectedUser", $s.attendType, $s.arr_selected_rcv, $s.arr_selected_cc);
//			nextPageData($rs,'reservApplySelectedUser','ReservBookingDetail', $s.attendType, $s.arr_selected_rcv, $s.arr_selected_cc); // 페이지 변경될때 데이터이동되도록 함수설정
		}
...
// {'code':'ETC','name':'기타'}
	...
	$s.btnLanguageSetting = function(event){
		...
		$rs.isDetailPage = true;
		$rs.currentDetailPageId = 'settings/settings_language_detail';
...
//		$rs.currentPageId = 'settings/settings_language_detail'
		nextPageData($rs,'initLanguageList','settingLanguage',$s.myLanguageCode,$s.languageList); // 페이지 변경될때 데이터이동되도록 함수설정
	};
	
	// 회사설정 페이지 이동
	$s.btnCompanySetting = function(event){
		$rs.isDetailPage = true;
		$rs.currentDetailPageId = 'settings/settings_company_detail';
...
//		$rs.currentPageId = 'settings/settings_company_detail'
		...
		nextPageData($rs,'initCompanySetting','settingCompany',$s.myCompanyCode,$s.companyList); // 페이지 변경될때 데이터이동되도록 함수설정
	}
	
	// 메인페이지 설정 이동
	$s.btnMainpageSetting = function(event){
		$rs.isDetailPage = true;
		$rs.currentDetailPageId = 'settings/settings_mainpage_detail';
...
//		$rs.currentPageId = 'settings/settings_mainpage_detail'
		...
		nextPageData($rs,'initMainpageList','settingMainpage',$s.myMainpageCode,$s.mainPageList); // 페이지 변경될때 데이터이동되도록 함수설정
	}
	// 결재처리 알림
...
//			$rs.currentPageId = 'mail/mail_write'
			nextPageData($rs,'initReplyForwardMailData','hallaMailWrite', $s.currSelectedMail, true, recipientArray, 2); // 페이지 변경될때 데이터이동되도록 함수설정
...
//			$rs.currentPageId = 'mail/mail_write'
			nextPageData($rs,'initReplyForwardMailData','hallaMailWrite', $s.currSelectedMail, true, recipientArray, 4); // 페이지 변경될때 데이터이동되도록 함수설정
			$s.dismissDlgMailPopupMenu();
			$s.currSelectedMail = undefined;
		});
	}
...
function PWDCheck(obj) {
	...
	for (intI = 0;intI < obj.length;intI++) {
		if (obj.substring(intI,intI+1) == obj.substring(0,1)) {
			intCnt = intCnt + 1;
	    }
	}
	  
	if (intCnt == obj.length) {
	    return false;
	}
