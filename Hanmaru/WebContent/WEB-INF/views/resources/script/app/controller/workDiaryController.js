...
//		$s.searchValue = '';
	...
	$rs.$on('initWorkList', function(event) {
		...
		var reqWorkTaskListData = {
			...
			EndDate:$s.endDate,
			PageNumber : 1,
			PageSize : 200,//9999, 2020.03.13 수정 - 리스트 최대갯수 200개만
		...
		$http(param).success(function(data) {
			...
			if($s.cal == undefined) {
				$s.cal = angular.element('#calendar').fullCalendar({
				    ...
				    //일정 more 기능. 추후 필요하면 추가할것.
//				    eventLimit: 5,
//				      eventLimitText: "more",
//				      //2020.03.18 추가. 일정 더보기
//				      eventLimitClick:function(info) {
//						    $s.$apply(function(){
//							    $s.isDlgScheduleDetail = true;
//							    
//							    switch( info.date.format( 'dddd' ) )
//							    {
//							    case 'Sunday':		$s.currSelectedDate = info.date.format('YYYY.MM.DD ') + '(일)';		break;
//							    case 'Monday':		$s.currSelectedDate = info.date.format('YYYY.MM.DD ') + '(월)';		break;
//							    case 'Tuesday':		$s.currSelectedDate = info.date.format('YYYY.MM.DD ') + '(화)';		break;
//							    case 'Wednesday':	$s.currSelectedDate = info.date.format('YYYY.MM.DD ') + '(수)';		break;
//							    case 'Thursday':	$s.currSelectedDate = info.date.format('YYYY.MM.DD ') + '(목)';		break;
//							    case 'Friday':		$s.currSelectedDate = info.date.format('YYYY.MM.DD ') + '(금)';		break;
//							    case 'Saturday':	$s.currSelectedDate = info.date.format('YYYY.MM.DD ') + '(토)';		break;
//							    }
//							    
//							    var formatDate = info.date.format();
//							    $s.selectDate = formatDate;
//
//								var mnt = moment( formatDate );
//								mnt.add('days', 1 );
...
//				      eventSources: parseCalData
				...
				$s.cal.fullCalendar('addEventSource', parseCalData, true);
	...
	$rs.$on('ChangeWorkList', function(event) {
		...
		//2020.03.13 수정 - api 호출후 이벤트 삭제하던것에서 swipe하고 바로 이벤트 삭제후 api호출.
		if($s.cal != undefined) {
			$s.cal.fullCalendar('removeEvents');
		}
		...
		$http(param).success(function(data) {
			...
			$s.cal.fullCalendar('addEventSource', parseCalData,true); //renderEvent는 각 이벤트 객체가 필요. addEventSource는 이벤트 배열이 필요.
			$s.cal.fullCalendar('gotoDate', mnt.format('YYYY-MM'));
		});
	});
...
//			var t1 = moment( $s.scheduleListData.Items[ i ].StartDate, 'YYYY-MM-DD' );
			...
			if( Range < $s.scheduleListData.Items[ i ].IntOfWidthDay ) {
				//2020.03.13 추가 - 추후 작업 메뉴 추가되면 수정할것. - if문 조건 주석처리하면됨. 
				if($s.scheduleListData.Items[i].WorkType != 'T'){
...
//일정 상세 보기
appHanmaru.controller('scheduleViewController', ['$scope', '$http', '$rootScope', '$sce','$timeout',function($s, $http,$rs,$sce,$timeout) {

	// 2019-03-06 PK broadcast
	$rs.$on('initScheduleView', function(event, item) {
		
		//2020.03.19 추가 - 변수 초기화 시점 변경.
		$s.bodyUrl = '';
		$s.approvalBody = '';
		
		// 2019-03-06 PK
		$s.ScheduleItem = item;
		$s.ScheduleViewData = undefined;
		...
		var reqScheduleParam = {
				LoginKey:$rs.userInfo.LoginKey,
				WorkID : $s.ScheduleItem.WorkID, //Id값은 일정, 작업 동일
...
//일정작성
appHanmaru.controller('scheduleWriteController', ['$scope', '$http', '$rootScope','$sce', function($s, $http, $rs,$sce) {
	
	$rs.$on('initScheduleWrite',function(event, changedata, changeviewdata){
		//2020.03.19 수정 - 변수 초기화 위치 변경.
		$s.showDialog = false;
...
//				$('.workViewContentsDiv').scrollTop(0);
//				console.log(resData);
			}else{
				$rs.result_message = $rs.translateLanguage('data_load_fail');
				$rs.dialog_toast = true;
