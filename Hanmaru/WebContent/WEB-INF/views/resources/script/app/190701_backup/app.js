...
.config(function($sceDelegateProvider, $compileProvider) {
	$sceDelegateProvider.resourceUrlWhitelist(['self','https://ep.halla.com/**','https://ep.hd-bsnc.com/**','/^(http|https)?:\/\/(.*\.)?shaking.com\/\?(.*)/']);
...
//	$rs.apiURL = "https://ep.hd-bsnc.com";
	...
	$s.general_id = 'kyungho.lee';//kyungho.lee //jina
	$s.general_pw = '777sapden3';//777sapden3 //qw123321
	$s.generalLogin_domain = 'halla.com';//halla.com //hd-bsnc.com
	$s.simpleLogin_domain = 'halla.com';//halla.com
...
//			androidWebView.checkPushMessage();
		} 
	...
	//푸시메세지 있는지 체크 
	//aos
//	window.responsePushMessage = function(title, message, messageKey, type) {
//		var account = localStorage.getItem("account");
//		if(account !=undefined && account != null){
//			$s.general_id = JSON.parse(account).ID;
//			$s.general_pw = JSON.parse(account).PW;
//		}
//		$s.performGeneralLogin();
//	}
	//ios
...
//		versionCheck();
	...
	function accessUser(){
		...
		$http(param).success(function(data) {
			...
			if(code == 1){
				//session storage에 id, pw 저장
//				localStorage.setItem("account",JSON.stringify({ID : $s.general_id,PW : $s.general_pw}));
...
//			$s.mailOptionShow = false;
	...
	$rs.loadMenu = function(menuName) {
		...
		if(menuName === 'reserv'){
			...
			$rs.$broadcast('initAttendanceList');
		}else{
...
appHanmaru.controller('attendanceController', ['$scope', '$http', '$rootScope', '$timeout', function($s, $http, $rs, $timeout) {
	...
	$rs.$on('initAttendanceList', function(event) {
		$rs.dialog_progress = true;
		$s.isAttendStart = false;
		$s.isAttendGoOut = false;
		$s.isAttendEnd = false;
		
		loadStartEndTime();
	});
	
	var loadStartEndTime = function(){
		var reqAttendData = {
				LoginKey:$rs.userInfo.LoginKey,
			};
			...
			$http(param).success(function(data) {
				...
				if(code == 1) {
					...
					$s.txtNowDate = $s.txtNowDate == '' ? moment(new Date()).format("YYYY-MM-DD") : $s.txtNowDate;
					$s.txtOTweek = resData.WEEK_OT;
					$s.txtOTmonth = resData.MONTH_OT;
					$s.seq = resData.SEQ;
					
					var tempStartTime = $s.isAttendGoOut ? resData.GOOUT_STARTTIME : resData.WORK_STARTTIME;
					var tempEndTime = $s.isAttendGoOut ? resData.GOOUT_ENDTIME : resData.WORK_ENDTIME;
					
					if(tempStartTime != ""){
						var startHour = tempStartTime.substr(0,2);
						var startMinute = tempStartTime.substr(2,2);
						var startTime = startHour + ":" + startMinute;
					}
					
					if(tempEndTime != ""){
						var endHour = tempEndTime.substr(0,2);
						var endMinute = tempEndTime.substr(2,2);
						var endTime = endHour + ":" + endMinute;
					}
					var startampm = parseInt(startHour) <12 ? "오전 " : "오후 ";
					var endampm = parseInt(endHour) <12 ? "오전 " : "오후 ";
					
					$s.txtStartTimeName = startTime != "" && startTime != undefined ? startampm + startTime : "-";
					$s.txtEndTimeName = endTime != "" && endTime != undefined ? endampm + endTime : "-";
				}
			}).then(function(){
				$timeout(function(){
					$rs.dialog_progress = false;
	...
	//출근
	$s.selectAttendStart = function(){
		$s.isAttendStart = true;
		$s.isAttendGoOut = false;
		$s.isAttendEnd = false;
	...
	//외출
	$s.selectAttendGoOut = function(){
		$s.isAttendStart = false;
		$s.isAttendGoOut = true;
		$s.isAttendEnd = false;
		
		loadStartEndTime();
	}
	//퇴근
	$s.selectAttendEnd = function(){
		$s.isAttendStart = false;
		$s.isAttendGoOut = false;
		$s.isAttendEnd = true;
	...
	$s.btnSetAttend = function(){
		if($s.isAttendStart){ //출근선택
			var req = {
				LoginKey : $rs.userInfo.LoginKey	
			};
			var checkAttend = confirm("출근 하시겠습니까?");
			if(checkAttend){
				var param = callApiObject('attendance', 'attendanceStart', req);
				$http(param).success(function(data){
					var code = parseInt(data.Code);
					if(code == 1){
						$rs.result_message = '출근 처리 되었습니다.';
						$rs.dialog_toast = true;
					}else{
						$rs.result_message = data.Result;
						$rs.dialog_toast = true;
					}
				}).then(function(){
					$timeout(function(){
		...
		}else if($s.isAttendGoOut){//외출 선택
			
			$rs.pushOnePage('pg_attendance_goout');
			$rs.$broadcast('initAttendanceGoout',$s.seq);
			
		}else if($s.isAttendEnd){//퇴근 선택
			var req = {
				LoginKey : $rs.userInfo.LoginKey	
			};
			var checkAttend = confirm("퇴근 하시겠습니까?");
			if(checkAttend){
				
				var param = callApiObject('attendance', 'attendanceEnd', req);
				$http(param).success(function(data){
					var code = parseInt(data.Code);
					if(code == 1){
						$rs.result_message = '퇴근 처리 되었습니다.';
						$rs.dialog_toast = true;	
					}else{
						$rs.result_message = data.Result;
						$rs.dialog_toast = true;	
...
//	$s.applyStartTime = function(e,time){
//		$s.txtStartTimeName = time.name;
//		$s.txtStartTimeValue = time.value;
//	}
//	
//	$s.applyEndTime = function(e,time){
...
//근태관리 - 외출사유 입력
appHanmaru.controller('attendanceGooutController', ['$scope', '$http', '$rootScope', '$timeout', function($s, $http, $rs, $timeout) {
	
	$s.attendGooutReason;
	$s.seq;//외출순번
	
	$rs.$on('initAttendanceGoout', function(event,seq) {
		$s.seq = seq;
	});
	...
	$s.gooutStart = function(){
		if($s.attendGooutReason == undefined || $s.attendGooutReason == ""){
			alert('외출 사유를 입력해 주세요.');
			return;
		}
		var req = {
			LoginKey : $rs.userInfo.LoginKey,
			Remark : $s.attendGooutReason,
			Seq : $s.seq
		};
		var checkGoout = confirm("외출 하시겠습니까?");
		if(checkGoout){
			$s.popPage('pg_attendance_goout');
			var param = callApiObject('attendance', 'attendanceGoOutStart', req);
			$http(param).success(function(data){
				var code = parseInt(data.Code);
				if(code == 1){
					$rs.result_message = '외출 처리 되었습니다.';
					$rs.dialog_toast = true;	
					$s.popPage('pg_attendance_goout');
				}else{
					$rs.result_message = data.Result;
	...
	$s.gooutEnd = function(){
		...
		var checkGoout = confirm("복귀 하시겠습니까?");
		if(checkGoout){
			$s.popPage('pg_attendance_goout');
			var param = callApiObject('attendance', 'attendanceGoOutEnd', req);
			$http(param).success(function(data){
				var code = parseInt(data.Code);
				if(code == 1){
					$rs.result_message = '복귀 처리 되었습니다.';
					$rs.dialog_toast = true;
					$s.popPage('pg_attendance_goout');
				}else{
					$rs.result_message = data.Result;
...
appHanmaru.controller('approvalController', ['$scope', '$http', '$rootScope', '$timeout', '$location', '$anchorScroll', '$window', function($s, $http, $rs, $timeout, $location, $anchorScroll, $window) {
	...
	$rs.$on('initApprovalList', function(event, data) {
		...
		if($rs.currSubMenu === 'ARRIVE'){ //미결함은 날짜제한 없음, 나머지 결재함은 1년전~당일 기준 날짜 설정 필요.
			$s.txtSearchStart = '';
			$s.txtSearchEnd = '';
		}else{
			var now = moment(new Date()).format("YYYY-MM-DD");
			var yearAgo = moment(new Date()).subtract(1, 'years').format("YYYY-MM-DD");
...
appHanmaru.controller('diaryScheduelController', ['$scope', '$http', '$rootScope', '$timeout', function($s, $http, $rs, $timeout) {
				...
				$s.cal = angular.element('#calendar').fullCalendar({
					...
					navLinkDayClick: function(date, jsEvent) {
					    ...
					    $s.$apply(function(){
						    ...
						    //$s.currSelectedDate = date.format('YYYY-MM-DD dddd') + 'Easy';
				    ...
				    //dayClick: function(calEvent, jsEvent, view) {
				    	...
				    	$s.$apply(function(){
				    		...
				    		//$s.currSelectedDate = calEvent.start.format('YYYY-MM-DD dddd') + 'Easy';
	...
	$s.fetchScheduleDetailList = function(date, EndDate) {
		...
		for(var i = 0; i < $s.scheduleListData.Count; i++) {
			...
			//console.log( "PK Date : " + $s.scheduleListData.Items[ i ].StartDate );
			//console.log( "PK Day Block : " + $s.scheduleListData.Items[ i ].IntOfWidthDay );
	...
	$s.btnShowScheduleWrite = function() {
		...
		$rs.$broadcast('ScheduleContentsReset');
	}
...
//		console.log( $s.scheduleContents );
		...
		if( $s.selectWithCalendar != undefined && $s.selectWithCalendar.Name != '' ) {
			calendar_ID = $s.selectWithCalendar.Key;
		}
